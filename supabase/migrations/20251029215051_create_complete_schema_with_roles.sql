/*
  # Create Complete Schema with Admin and Operator Roles

  ## Overview
  This migration creates the foundation for a sugarcane emissions tracking system
  with role-based access control (admin and operator roles).
  ...
  - `role` (text) - User role (admin or operator)
  ...
  - Admins can view and manage all users and their data
  - Operators can only view and manage their own data
  ...
  4. RLS policies restrict access based on user role and ownership
*/
...
-- Hectares policies
CREATE POLICY "Admins can view all hectares"
...
CREATE POLICY "Operators can view own hectares"
  ON hectares FOR SELECT
  TO authenticated
  USING (
    user_id = auth.uid()
...
CREATE POLICY "Operators can insert own hectares"
...
CREATE POLICY "Operators can update own hectares"
  ON hectares FOR UPDATE
  TO authenticated
  USING (user_id = auth.uid())
  WITH CHECK (user_id = auth.uid());

CREATE POLICY "Operators can delete own hectares"
...
CREATE POLICY "Operators can view own emissions"
  ON emissions FOR SELECT
  TO authenticated
  USING (
    user_id = auth.uid()
...
CREATE POLICY "Operators can insert own emissions"
...
CREATE POLICY "Operators can update own emissions"
  ON emissions FOR UPDATE
  TO authenticated
  USING (user_id = auth.uid())
  WITH CHECK (user_id = auth.uid());

CREATE POLICY "Operators can delete own emissions"
...
CREATE POLICY "Operators can view own tokens"
  ON tokens FOR SELECT
  TO authenticated
  USING (
    user_id = auth.uid()
...
CREATE POLICY "Operators can insert own tokens"
...
CREATE POLICY "Operators can update own tokens"
  ON tokens FOR UPDATE
  TO authenticated
  USING (user_id = auth.uid())
  WITH CHECK (user_id = auth.uid());

CREATE POLICY "Operators can delete own tokens"
